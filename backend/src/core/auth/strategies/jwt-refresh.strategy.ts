...
export class JwtRefreshStrategy extends PassportStrategy(Strategy, 'jwt-refresh') {
  constructor(
    ...
    const cookieName = configService.get<string>('auth.cookieName') || 'ecomcx_session';
  ...
  async validate(req: Request, payload: any) {
    ...
    const cookieName = this.configService.get<string>('auth.cookieName') || 'ecomcx_session';
    const refreshToken =
    ...
    if (!refreshToken) {
      ...
      throw new CustomApiException(ErrorCode.AUTH_REFRESH_TOKEN_MISSING, message, HttpStatus.UNAUTHORIZED);
    }
