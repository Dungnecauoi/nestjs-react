...
let JwtAuthGuard = class JwtAuthGuard extends (0, passport_1.AuthGuard)('jwt') {
    ...
    handleRequest(err, user, info) {
        if (err || !user) {
            ...
            const errorCode = isExpired ? error_code_enum_1.ErrorCode.AUTH_TOKEN_EXPIRED : error_code_enum_1.ErrorCode.AUTH_UNAUTHORIZED;
            const message = isExpired
                ? 'Access Token đã hết hạn. Vui lòng làm mới Token'
                : 'Phiên truy cập không hợp lệ hoặc chưa được đăng nhập';
            throw new custom_api_exception_1.CustomApiException(errorCode, message, common_1.HttpStatus.UNAUTHORIZED);
        }
        return user;
    }
};
